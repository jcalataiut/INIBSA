...
if analisis_mode == "Visión Global":
    ...
    with colB:
        st.subheader("Potencial vs. Ventas (Anualizadas) (Todos los Clientes)")
        
        # Selector de año
        lista_anios = ['Promedio Histórico'] + sorted(df['Fecha'].dt.year.unique().tolist())
        selected_anio = st.selectbox("Selecciona Año a comparar", lista_anios)
        
        if selected_anio == 'Promedio Histórico':
            ...
            title_scatter = "Captura vs Potencial (Promedio Histórico)"
        else:
            # Filtrar por el año seleccionado
            df_year = df[df['Fecha'].dt.year == selected_anio]
            ventas_cliente = df_year.groupby(['Id_Cliente', 'Familia_Potencial']).agg(
                Ventas_Anuales_Medias=('Valores_H', 'sum'),
                Potencial_EUR=('Potencial_EUR', 'first')
            ).reset_index()
            title_scatter = f"Captura vs Potencial (Solo {selected_anio})"
        
        fig_scatter = px.scatter(ventas_cliente, x='Potencial_EUR', y='Ventas_Anuales_Medias', color='Familia_Potencial',
                                 hover_data=['Id_Cliente'], title=f"{title_scatter} (Bajo la diagonal = No capturado)")
        
        # Añadir línea y=x (diagonal donde Ventas = Potencial)
        max_val = ventas_cliente[['Potencial_EUR', 'Ventas_Anuales_Medias']].max().max()
